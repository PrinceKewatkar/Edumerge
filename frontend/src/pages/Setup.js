...
const Setup = () => {
  ...
  const fetchDropdownData = async () => {
    ...
    try {
      if (section === 'campuses') {
        ...
        dropdowns.institutions = institutionsRes.data;
        dropdowns.departments = departmentsRes.data;
      } else if (section === 'program-quotas') {
        const [institutionsRes, programsRes, quotasRes] = await Promise.all([
          api.get('/institutions/'),
          api.get('/programs/'),
          api.get('/quotas/')
        ]);
  ...
  const getInitialForm = () => {
    switch (section) {
      ...
      case 'programs':
        return { institution: '', department: '', name: '', code: '', course_type: 'UG', entry_type: 'Regular', total_intake: 0 };
      case 'academic-years':
        return { year: '', is_active: true };
      case 'quotas':
        return { name: '', code: '' };
      case 'program-quotas':
        return { institution: '', program: '', quota: '', seats_allocated: 0 };
      default:
        return {};
    }
  };
  ...
  const openModal = (item = null) => {
    ...
    if (item && (section === 'programs' || section === 'program-quotas')) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      // Remove institution field before submitting (it's only used for frontend filtering)
      const submitData = { ...formData };
      if (section === 'programs' || section === 'program-quotas') {
        delete submitData.institution;
      }

      if (editingItem) {
        await api.patch(`/${endpoint}/${editingItem.id}/`, submitData);
      } else {
        ...
        return (
          <>
            <div className="form-group">
              ...
              <select
                className="form-select"
                value={formData.institution || selectedInstitution}
                onChange={(e) => {
                  setSelectedInstitution(e.target.value);
                  setFormData({ ...formData, institution: e.target.value, department: '' });
                }}
                required
              >
                <option value="">Select Institution</option>
            ...
            <div className="form-group">
              ...
              <select
                ...
                required
                disabled={!formData.institution && !selectedInstitution}
              >
                <option value="">Select Department</option>
                {dropdownData.departments
                  ?.filter((dept) => dept.institution === parseInt(formData.institution || selectedInstitution))
                  .map((dept) => (
          ...
          <>
            <div className="form-group">
              ...
              <select
                ...
                onChange={(e) => {
                  setSelectedInstitution(e.target.value);
                  setFormData({ ...formData, institution: e.target.value, program: '' });
                }}
                required
              >
                <option value="">Select Institution</option>
            ...
            <div className="form-group">
              ...
              >
                ...
                {dropdownData.programs
                  ?.filter((prog) => prog.institution === parseInt(formData.institution || selectedInstitution))
                  .map((prog) => (
